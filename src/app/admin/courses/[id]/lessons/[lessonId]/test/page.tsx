...
type LessonTestEditorPageProps = {
...
export default async function LessonTestEditorPage({ params }: LessonTestEditorPageProps) {
  await requireAdmin();
  ...
  if (!lesson.test) {
    redirect(`/admin/courses/${id}/lessons/${lessonId}`);
  }
  ...
  return (
    <div className="stack">
      <PageHeader
        actions={
          <Link className="btn btn-secondary" href={`/admin/courses/${id}/lessons/${lessonId}`}>
            ← К уроку
          </Link>
        }
        breadcrumbs={[
          { label: "Курсы", href: "/admin/courses" },
          { label: lesson.course.title, href: `/admin/courses/${id}` },
          { label: lesson.title, href: `/admin/courses/${id}/lessons/${lessonId}` },
          { label: "Тест" }
        ]}
        eyebrow="Тест урока"
      ...
      <div className="editor-grid">
        <div className="stack">
          <section className="card stack">
            ...
            <form action={updateTestAction} className="admin-form">
              ...
              <div className="field">
                <label htmlFor="lesson-test-title">Название теста</label>
                <input className="input" defaultValue={test.title} disabled={isLocked} id="lesson-test-title" name="title" required />
              </div>
              <div className="row">
        ...
        <section className="card stack">
          <div>
            <h3>Вопросы теста</h3>
            <p className="text-muted">Порядок вопросов и preview правильных ответов</p>
          </div>
                ...
                <article className="question-row" key={question.id}>
                  ...
                  <div className="question-row-actions">
                    ...
                    {isLocked ? (
                      ...
                      <Link className="btn btn-icon btn-ghost" href={`/admin/courses/${id}/lessons/${lessonId}/test/questions/${question.id}`} title="Редактировать">
                        <Edit2 size={17} />
                      </Link>
                    )}
