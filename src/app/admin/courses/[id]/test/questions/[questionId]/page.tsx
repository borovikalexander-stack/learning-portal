...
export default async function QuestionEditorPage({ params }: QuestionEditorPageProps) {
  ...
  if (!question || question.test.course.id !== id || question.test.lessonId) {
    notFound();
  }
  ...
  return (
    <div className="stack">
      <PageHeader
        actions={
          <Link className="btn btn-secondary" href={`/admin/courses/${id}/test`}>
            ← К тесту
          </Link>
        }
        breadcrumbs={[
          ...
          { label: "Тест", href: `/admin/courses/${id}/test` },
