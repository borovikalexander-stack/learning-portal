...
export default async function AdminReviewPage() {
  ...
  const pendingAnswers = await prisma.answerAttempt.findMany({
    ...
    include: {
      ...
      attempt: {
        include: {
          user: { select: { id: true, firstName: true, lastName: true, avatarUrl: true, department: { select: { name: true } } } },
  ...
  return (
    <PageReveal className="stack">
      <div data-reveal>
      <PageHeader eyebrow="Управление" title="Проверка открытых ответов">
        <p className="text-muted">Открытые ответы тестов от менеджеров всей компании</p>
      </PageHeader>
      </div>
      ...
      {pendingAnswers.length === 0 ? (
        <section className="card stack motion-card" data-reveal>
          <div className="empty-state">
            <h3>Открытых ответов на проверке нет</h3>
                  ...
                  <div className="row" style={{ gap: 12 }}>
                    ...
                    <div>
                      ...
                      <div className="text-muted small">
                        {a.attempt.user.department?.name ?? "Без отдела"} · {formatDate(a.attempt.submittedAt)}
                      </div>
                    </div>
                  </div>
                ...
                <form action={reviewAnswerAction} className="stack">
                  ...
                  <div className="field">
                    ...
                    <textarea
                      ...
                      id={`comment-${a.id}`}
                      name="comment"
                      placeholder="Что отметить менеджеру"
                      rows={2}
                    />
                  </div>
                  <div className="row" style={{ flexWrap: "wrap" }}>
                    ...
                    <Link className="btn btn-ghost" href={`/admin/users/${a.attempt.user.id}`}>
                      Карточка менеджера
                    </Link>
                  </div>
                </form>
